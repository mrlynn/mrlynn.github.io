...
I built the version I wanted when I was learning this stuff. One domain. Four routes, each adding exactly one new idea. And a course you can actually read and teach from, not just clone and run once.
...
That incident is not decoration. It is why the triage schema has a calibrated confidence score. It is why `/v1/resolve` returns the full tool trace. And it is why the eval set includes a deliberately ambiguous case that is allowed to fail.
